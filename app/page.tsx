...
function toISODate(raw: unknown): string | null {
  ...
  // YYYY-MM-DD (or longer timestamps)
...
function parseBookingsCSV(text: string) {
  ...
  for (const row of data) {
    ...
    const property =
      (propRaw ? String(propRaw).trim() : "Unknown") || "Unknown";
...
function parseExpensesCSV(text: string) {
  const data = parseCSV(text);
  ...
  for (const row of data) {
    ...
    const property =
      (propRaw ? String(propRaw).trim() : "Unknown") || "Unknown";
    const category =
      (catRaw ? String(catRaw).trim() : "Uncategorized") || "Uncategorized";
...
export default function Home() {
  ...
  async function loadBookings(files: FileList | null) {
    ...
    for (const f of Array.from(files)) {
      ...
      const res = parseBookingsCSV(text);
      rows += res.rowsSeen;
      created += res.created;
      all.push(...res.txns);
    }
